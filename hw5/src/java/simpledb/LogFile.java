...
public class LogFile {
    ...
    */
    ...
    private void rollback(long tid) throws NoSuchElementException, IOException {
        Long firstLogRecord = tidToFirstLogRecord.get(Long.valueOf(tid));
        ...
        while(raf.getFilePointer() >= tidToFirstLogRecord.get(tid)) {
            ...
            if(entryType == UPDATE_RECORD && raf.readLong() == tid) {
                ...
                ((HeapFile) Database.getCatalog().getDbFile(beforeImage.getId()
                        .getTableId())).writePage(beforeImage);
                Database.getBufferPool().replacePage(beforeImage.getId(), beforeImage);
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                if (checkpointOffset >= 0) {
                    ...
                    // transaction id
                    raf.readLong();
                    int numberOfActiveTransactions = raf.readInt();
                    for (int i = 0; i < numberOfActiveTransactions; i++) {
                        long activeTransactionId = raf.readLong();
                        // Offset of first log related to this transaction
                        tidToFirstLogRecord.put(activeTransactionId,
                                raf.readLong());
                    }
                    // End of checkpoint log - offset
                    raf.readLong();
                }
                while (raf.getFilePointer() < raf.length()) {
                    ...
                    switch (transactionType) {
                        case ABORT_RECORD:
                            ...
                            raf.readLong();
                            tidToFirstLogRecord.remove(transactionId);
                            break;
                        case BEGIN_RECORD:
                            tidToFirstLogRecord.put(transactionId, raf.readLong());
                            break;
                        case COMMIT_RECORD:
                            raf.readLong();
                            tidToFirstLogRecord.remove(transactionId);
                        ...
                        case UPDATE_RECORD:
                            ...
                            Database.getBufferPool().replacePage(beforeImage.getId(), afterImage);
                            raf.readLong();
                            break;
                        default:
                            break;
                ...
                for (Long loserTransactionId : tidToFirstLogRecord.keySet()) {
                    rollback(loserTransactionId);
                // some code goes here
                }
            }
         }
